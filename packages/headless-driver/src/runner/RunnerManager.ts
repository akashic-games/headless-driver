...
export interface CreateRunnerParameters {
	...
	/**
	 * 信頼されたコンテンツであるかどうか。
	 * `true` の場合、対象のゲームコンテンツを「信頼されたもの」として実行します。信頼されたコンテンツであれば、エンジンモジュールから外部モジュールまたは組み込みモジュールへのアクセスが許可されます。
	 * `false` の場合、コンテンツまたはエンジンモジュールでの動作が一部制限されます (外部モジュールまたは組み込みモジュールへのアクセスが制限されます)。
	 * 初期値は `false` 。
	 */
	trusted?: boolean;
	/**
