...
export class PlayManager {
	...
	async createPlay(playLocation: PlayManagerParameters, playlog?: DumpedPlaylog): Promise<string> {
		...
		if (playlog) {
			...
			await new Promise<void>((resolve, reject) => amflow.open(playId, (e: Error) => (e ? reject(e) : resolve())));
			await new Promise<void>((resolve, reject) => amflow.authenticate(token, (e: Error) => (e ? reject(e) : resolve())));
			await new Promise<void>((resolve, reject) =>
				amflow.putStartPoint(playlog.startPoints[0], (e: Error) => (e ? reject(e) : resolve()))
			);
